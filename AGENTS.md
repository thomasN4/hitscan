...
- **Dynamic index reads must prove their miss case, or make the miss unrepresentable.** `noUncheckedIndexedAccess` makes an array read `T | undefined`. Prefer removing the case: where the table is fixed-size, type it as a TUPLE and its index as a union (`WEAPONS: readonly [WeaponDef, WeaponDef]`, `game.slot: WeaponSlot`) — tuple-plus-literal-union indexing is exempt from the flag, so `WEAPONS[game.slot]` is a plain `WeaponDef` and there is no miss to guard. Where the index is genuinely unbounded (`zoomFovs[game.zoomLevel]`), decide explicitly: `weapons.ts:aimFovFor` clamps because it owes its caller a number, the HUD zoom label degrades to empty because a view must not throw mid-frame, and a narrowing helper that throws a named error is right where a miss means corrupted state. NO `?? defaultValue` fallbacks — a fallback invents a value the old code never produced. Bare `!` assertions are for bound-guarded reads (`impacts[i]` inside its own loop, `hits[0]` after a length check) and tests.
