...
- **No module-scope side effects that touch the engine or the DOM.** A module needing either exposes an `init*()` function that `main.ts` calls in order. Current order, which `main.ts` documents inline:
  ...
  ```
  initEngine() → initHUD() → initWeaponViewmodels() → BUILDERS[session.map]() → respawn() → initPlayControl() → initTouchControls() (touch mode) → initMenus() → loop
  ```
...
- **Touch input writes the same slices the mouse and keys do**, so no stage downstream knows which device acted. The stick writes `input.moveX`/`moveY` (which replace WASD in `player.ts` when nonzero) and `input.running` at its rim; the look surfaces and mouse share `sim/look.ts:applyLook`; ADS and scope zoom share `weapons.ts:tryRaiseSights` / `cycleZoom` with RMB and the wheel. Add a new gate to those shared functions, never to one device's handler.
...
- `window.__cs` in main.ts is a debug/testing hook relied on by the smoke test — keep it exporting `{ game, weapon, player, bots, bulletHoles, colliders, elevators, gameTime, dom, nav: { route, transportRoute, grid } }` (its shape is declared on `Window` in main.ts).
