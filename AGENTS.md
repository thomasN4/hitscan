...
- **The per-frame stage order in `main.js:animate()` is load-bearing:**
  ...
  `updateWeapon` consumes the blends `updateMovement` writes and decays `game.recoil`; `updateCamera` and `updateViewmodel` then read that post-decay recoil, so camera, viewmodel and bullets agree within a frame. Reordering aims the camera a frame ahead of the shots (`5e004a5`). Keep the sequence flat in `animate()` — do not nest one stage inside another.
