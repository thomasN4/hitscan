...
| v8 | 2026-09-12 | Review-driven corrections to the v7 fallback. The executor retry gets only the turn's remaining watchdog budget, never a fresh one; a failed append-mode recovery attempt is cut back out of `events.jsonl` (its bytes stay in its attempt file) so the gate keeps judging one stream. The reviewer now distinguishes a skipped primary (notice, fallback runs directly), a failed primary (warning, fallback retries) and no key at all (error naming both secrets) instead of reporting exit 2 for all three. |
