  ...
  try {
    ...
    if (sprintCheck) {
      const qcancel = await page.evaluate(async () => {
        ...
        // then raise the sights. The fresh RMB press is REFUSED while the
        // reload runs — ADS is impossible until it finishes — so the reticle
        // never comes up over a running reload and the reload is not cancelled.
        // Released again immediately — input.aiming is raw button state only
        // outside a reload, and a held RMB would ADS the moment the reload
        // completes instead of staying hip for the closing refill below.
        window.dispatchEvent(new KeyboardEvent('keydown', { code: 'KeyR' }));
        await wait(150);
...
async function runShotgunCheck() {
  ...
  try {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.dispatchEvent(new MouseEvent('mousedown', { button: 2 }));
      await frame();
      const afterAimCancel = { mag: cs.weapon.mag, reloading: cs.weapon.reloading, aiming: cs.game.aiming };
      window.dispatchEvent(new MouseEvent('mouseup', { button: 2 }));
      return {
        primary: cs.game.primary,
        secondary: cs.game.secondary,
...
async function runBotWeaponsCheck() {
  ...
  try {
    // ---- A. 'mixed' arms a varied field of primaries.
    ...
    // A mixed wave draws primaries only: sidearms arrive via the secondary
    // position and the blade via every loadout's fallback.
    const unknown = mixed.before.filter(w => !PRIMARIES.includes(w));
    ...
    // `drainTo` (a sidearm) first burns the primary downrange until the bot
    // swaps to it: sidearms are secondary-only now, so no query can open on
    // one directly. The shotgun primary is the fastest burn (7+28 rounds).
    const combat = async (weapon, botZ, playerZ, simSeconds, drainTo = null) => page.evaluate(
      async (weapon, botZ, playerZ, simSeconds, drainTo) => {
        const cs = window.__cs;
        const frame = () => new Promise(r => requestAnimationFrame(r));
        const LANE_X = 12;
        const blocked = (x, z) => cs.colliders.some(c =>
        ...
        const bot = cs.bots[0];
        if (drainTo !== null) {
          // Burn the primary the way a match does: pinned in place, shooting
          // down the lane at an unkillable player until the swap lands.
          cs.player.pos.set(LANE_X, cs.player.eyeHeight, playerZ);
          cs.player.alive = true;
          const drainWall = performance.now();
          while (bot.weapon !== drainTo && performance.now() - drainWall < 240000) {
          ...
          if (bot.weapon !== drainTo) return { fail: `bot never swapped to ${drainTo}, stuck on ${bot.weapon}` };
        }
      ...
      }, weapon, botZ, playerZ, simSeconds, drainTo);

    const load = async (query) => {
      await page.goto(mapUrl(`/?map=arena&tbots=1&ctbots=0&time=600&${query}`), { waitUntil: 'networkidle0', timeout: 20000 });
      await new Promise(r => setTimeout(r, 1200));
    };
    ...
    // the only difference is what they are holding. The revolver is a
    // sidearm, so its leg opens on a shotgun primary and drains to it —
    // which also proves the dry swap the default match relies on.
    await load('tweap=smg');
    const smg = await combat('smg', -15, -7, 26);
    if (smg.fail) throw new Error(smg.fail);
    await load('tweap=shotgun&tsec=revolver');
    const revolver = await combat('revolver', -15, -7, 26, 'revolver');
