...
/**
 * Pick the best respawn candidate CoD-style: far from enemies, near owned
 * flags. Score = 2 × nearest-enemy distance − 0.2 × nearest-owned-flag
 * distance; with no owned flags the second term is 0 (pure far-from-enemy),
 * with no enemies the first term reads the map diagonal. Returns the winning
 * INDEX, or -1 when there are no candidates. Ties go to the lowest index, so
 * the choice is deterministic. Standability is the CALLER's job (rejection
 * sampling against colliders) — this only ranks.
 */
export function scoreDomSpawn(
  candidates: readonly SpawnCandidate[],
  enemies: readonly { x: number; z: number }[],
  ownedFlags: readonly { x: number; z: number }[],
): number {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    let enemyD = 120; // ~ the map diagonal: nobody near means nobody matters
    for (const e of enemies) enemyD = Math.min(enemyD, Math.hypot(c.x - e.x, c.z - e.z));
    let flagD = 0;
    if (ownedFlags.length > 0) {
      flagD = Infinity;
      for (const f of ownedFlags) flagD = Math.min(flagD, Math.hypot(c.x - f.x, c.z - f.z));
    }
    const score = 2 * enemyD - 0.2 * flagD;
    if (score > bestScore) {
      bestScore = score;
      bestIdx = i;
    }
